...
app.get("/api/stats", async (req, res) => {
  try {
    ...
    res.json({
      ...
      fakeDetections: fake,
      safeDetections: total - fake,
      detectionRate: total ? ((fake / total) * 100).toFixed(1) : 0,
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get("/api/trends", async (req, res) => {
  try {
    ...
    const formatted = trends.map((t) => ({
      time: `${t._id.toString().padStart(2, "0")}:00`,
...
app.get("/api/recent-alerts", async (req, res) => {
  try {
    const recent = await Scan.find({ isLegitimate: false })
      .sort({ timestamp: -1 })
      .limit(10);
    const alerts = recent.map((scan) => ({
      id: scan._id,
      time: getTimeAgo(scan.timestamp),
      message: `Suspicious: ${extractDomain(scan.url)}`,
      type: scan.confidence > 0.9 ? "critical" : "high",
    }));
    res.json(alerts);
  } catch (error) {
    res.status(500).json({ error: error.message });
