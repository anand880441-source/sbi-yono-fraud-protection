...
const scanSchema = new mongoose.Schema({
    ...
    source: { type: String, enum: ['dashboard', 'extension', 'whatsapp', 'mobile', 'bulk'], default: 'dashboard' },
...
const reportSchema = new mongoose.Schema({
    ...
    source: { type: String, enum: ['dashboard', 'extension', 'whatsapp', 'mobile'], default: 'dashboard' },
...
app.post("/api/detect", async (req, res) => {
    try {
        ...
        console.log(`📊 Scan saved: ${url} -> ${mlData.is_legitimate ? 'Safe' : 'Fake'} (${(mlData.confidence * 100).toFixed(0)}%)`);
...
app.post("/api/report", async (req, res) => {
    try {
        ...
        // Also forward to ML service for logging
        try {
...
app.post("/api/detect-bulk", async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: "Failed to process bulk detection" });
    }
});
...
app.get("/api/stats", async (req, res) => {
    try {
        ...
        const recentScans = await Scan.countDocuments({ timestamp: { $gte: last24h } });
        const recentFake = await Scan.countDocuments({ timestamp: { $gte: last24h }, isLegitimate: false });
        ...
        res.json({
            ...
            detectionRate: totalScans > 0 ? ((fakeDetections / totalScans) * 100).toFixed(1) : 0,
    ...
    try {
        ...
        const trends = await Scan.aggregate([
            ...
            { $group: {
                ...
                detections: { $sum: 1 },
                blocked: { $sum: { $cond: [{ $eq: ["$isLegitimate", false] }, 1, 0] } }
            }},
...
app.get("/api/top-threats", async (req, res) => {
    try {
        ...
        const formattedThreats = threats.map(t => {
            const risk = t.count > 100 ? 'Critical' : t.count > 50 ? 'High' : t.count > 20 ? 'Medium' : 'Low';
            return {
...
app.get("/api/threat-types", async (req, res) => {
    try {
        ...
        const distribution = Object.entries(threatTypes).map(([name, value]) => ({
            ...
            color: name === 'Fake YONO Apps' ? '#f44336' : 
                   name === 'Phishing Links' ? '#ff9800' : 
                   name === 'APK Malware' ? '#9c27b0' : '#2196f3'
        }));
...
app.get("/api/recent-alerts", async (req, res) => {
    try {
        ...
        const alerts = recentFake.map(scan => ({
            ...
            type: scan.confidence > 0.9 ? 'critical' : scan.confidence > 0.7 ? 'high' : 'medium',
            url: scan.url
        }));
